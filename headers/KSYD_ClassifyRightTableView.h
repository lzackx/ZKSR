//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <UIKit/UITableView.h>

#import "UITableViewDataSource-Protocol.h"
#import "UITableViewDelegate-Protocol.h"

@class NSMutableArray, NSString;

@interface KSYD_ClassifyRightTableView : UITableView <UITableViewDelegate, UITableViewDataSource>
{
    _Bool _isDetail;
    _Bool _isLeftSpace;
    CDUnknownBlockType _CellSelectedBlock;
    NSMutableArray *_dataArray;
}

- (void).cxx_destruct;
@property(copy, nonatomic) CDUnknownBlockType CellSelectedBlock; // @synthesize CellSelectedBlock=_CellSelectedBlock;
@property(retain, nonatomic) NSMutableArray *dataArray; // @synthesize dataArray=_dataArray;
- (id)initWithFrame:(struct CGRect)arg1 style:(long long)arg2;
@property(nonatomic) _Bool isDetail; // @synthesize isDetail=_isDetail;
@property(nonatomic) _Bool isLeftSpace; // @synthesize isLeftSpace=_isLeftSpace;
- (long long)numberOfSections;
- (void)setSelectedRow:(long long)arg1;
- (id)tableView:(id)arg1 cellForRowAtIndexPath:(id)arg2;
- (void)tableView:(id)arg1 didSelectRowAtIndexPath:(id)arg2;
- (long long)tableView:(id)arg1 numberOfRowsInSection:(long long)arg2;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

