//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <UIKit/UIApplication.h>

@class NSMutableDictionary;

@interface UIApplication (GDTCacheImage)
- (void)GDTfunctioni2WnKp:(id)arg1 forRequest:(id)arg2;
- (id)GDTfunctioni51EiP:(id)arg1;
- (void)GDTfunctioni65qQT:(id)arg1;
- (void)GDTfunctionn5xOD5;
- (id)GDTfunctiont302A9:(id)arg1;
@property(readonly, nonatomic) NSMutableDictionary *GDTfunctionu0I8nQ;
- (void)GDTfunctionv2jxNH;
- (unsigned long long)GDTfunctionz4u26W:(id)arg1;
- (void)bu_zf_cacheFailRequest:(id)arg1;
@property(readonly, nonatomic) NSMutableDictionary *bu_zf_cacheFaileTimes;
- (void)bu_zf_cacheImage:(id)arg1 forRequest:(id)arg2;
- (id)bu_zf_cacheImageForRequest:(id)arg1;
- (unsigned long long)bu_zf_failTimesForRequest:(id)arg1;
- (void)setBu_zf_cacheFaileTimes:(id)arg1;
- (void)setGDTfunctionu0I8nQ:(id)arg1;
- (void)zf_clearCache;
- (void)zf_clearDiskCaches;
@end

